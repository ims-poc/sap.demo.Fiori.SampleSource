jQuery.sap.declare("sap.demo.StockTransfer.util.common");
...
sap.demo.StockTransfer.util.common = {
	...
	createValueHelpRequestForSupplierSite : function(oThis) {
		...
		var oValueHelpDialog = new sap.ui.comp.valuehelpdialog.ValueHelpDialog({
			...
			ok: function(oControlEvent) {
				...
				oModel.attachRequestCompleted(function(){
					...
					for(var i =  0; i < aData.length; i++){
						if(aData[i].StoreID === sKey) {
							sap.ui.getCore()._mockDataModel.getData().ProductCollection[sap.ui.getCore()._selectedIndex].SupplierSite = aData[i].StoreName;
							sap.ui.getCore()._mockDataModel.refresh();
							oValueHelpDialog.close();
							return;
						}
					}
	...
	setEnabledForSupplierSite : function(sSite) {
		if(sSite === "" || sSite === undefined) {
			return true;
		} else {
			return false;
		}
